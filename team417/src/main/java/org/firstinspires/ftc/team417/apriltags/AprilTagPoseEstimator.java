...
public class AprilTagPoseEstimator {
    ...
    public InfoWithDetection chooseBestAprilTag(ArrayList<InfoWithDetection> iwdList) {
        ...
        for (int i = 0; i < iwdListSize; i++) {
            currentIwd = iwdList.get(i);
            if (!isEpsilonEquals(currentIwd.info.sideLength, largestSize) && currentIwd.info.sideLength < largestSize) {
                iwdList.remove(i);
                i--;
            }
            iwdListSize = iwdList.size();
    ...
    public Pose2d estimatePose() {
        if (robotPoseEstimate != null) {
            return new Pose2d(robotPoseEstimate.position.x, robotPoseEstimate.position.y, robotPoseEstimate.heading.log());
        } else {
            return null;
        }
    }
