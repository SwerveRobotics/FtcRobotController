...
public class LoonyTune extends LinearOpMode {
    ...
    class Query {
        ...
        boolean previewPrompt(Action previewTrajectory, String message) {
            ...
            while (!isStopRequested() && !menu.cancel()) {
                previewer.update();
                message(message);
                if (menu.accept()) {
                    success = true;
        ...
        boolean drivePrompt(String message, boolean exit) {
            ...
            while (!menu.accept()) {
                ...
                if ((exit && menu.exit()) || (!exit && menu.cancel())) {
                    success = false;
                    break;
                }
                updateGamepadDriving();
    ...
    private boolean runCancelableAction(Action action) {
        ...
        while (opModeIsActive() && !menu.cancel()) {
    ...
    void wheelDebugger() {
        ...
        stopMotors();
        while (opModeIsActive() && !menu.exit()) {
    ...
    void driveTest() {
        ...
        while (opModeIsActive() && !menu.exit()) {
            ...
            if (menu.leftBumper() || menu.rightBumper()) { // Debug wheels
                wheelDebugger();
                drive.setPose(homePose);
                baselinePose = null;
                previousPose = homePose;
            }
            if (menu.yButton()) { // Set baseline at home
                drive.setPose(homePose);
                previousPose = homePose;
    ...
    class NumericInput {
        ...
        String update() {
            ...
            int input = menu.gamepad.dpad_up ? 1 : (menu.gamepad.dpad_down ? -1 : 0); // -1, 0 or 1
            if (input != lastInput) {
                nextAdvanceTime = time() + INITIAL_DELAY;
                lastInput = input;
    ...
    void runTrajectory(Action action, String promptMessage) {
        ...
        while (opModeIsActive() && !menu.cancel()) {
            ...
            if (menu.leftBumper() || menu.rightBumper()) {
